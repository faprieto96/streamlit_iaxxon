...
Nombre_complejo_instalacion = "Camping Arcoiris"
...
def get_data(time_period):
    ...
    query= f'from (bucket: "Camping_Arco_Iris")\
...
def get_kwh(time_period):
    ...
    |> filter(fn: (r) => r["_field"] == "TCAP" or r["_field"] == "TAC1" or r["_field"] == "fan")\
    |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")\
    |> map(fn: (r) => ({{_start: r._start,_stop: r._stop,result: r.result,_time: r._time,_measurement: r._measurement,_field: "energia_kwh",_value: (float(v: r.TCAP) - float(v: r.TAC1)) * float(v: r.fan) * 0.027}}),)\
...
query_api = client.query_api()
query_fan = f'''from(bucket: "Camping_Arco_Iris")\
    |> range(start: -15m)\
