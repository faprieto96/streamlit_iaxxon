...
Nombre_complejo_instalacion = "Piscina de Priego"
...
def get_kwh(time_period):
    ...
    |> filter(fn: (r) => r["_field"] == "TCAP" or r["_field"] == "TAC1" or r["_field"] == "heat")\
    |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")\
    |> map(fn: (r) => ({{_start: r._start,_stop: r._stop,result: r.result,_time: r._time,_measurement: r._measurement,_field: "energia_kwh",_value: (float(v: r.TCAP) - float(v: r.TAC1)) * float(v: r.heat) * 0.027}}),)\
...
with col2:
    st.metric(label="Temperatura Interior", value=f"{df.TAC1.iloc[-1]} °C")
    st.metric(label="Temperatura Exterior", value=f"{df.TINT.iloc[-1]} °C")
