...
Nombre_complejo_instalacion = "Piscina - Villanueva de Córdoba"
...
def get_data(time_period):
    ...
    query= f'from (bucket: "piscina_villanueva")\
...
query_api = client.query_api()
query_fan = f'''from(bucket: "piscina_villanueva")\
    |> range(start: -15m)\
...
query_pump = f'''from(bucket: "piscina_villanueva")\
    |> range(start: -15m)\
    |> filter(fn: (r) => r._measurement == "prueba")\
